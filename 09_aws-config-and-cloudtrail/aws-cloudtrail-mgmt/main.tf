...
# CloudWatch Metric Filter 생성, 루트 계정 사용을 감지
resource "aws_cloudwatch_log_metric_filter" "root_account_usage" {
  name           = "RootAccountUsage"
  log_group_name = aws_cloudwatch_log_group.cloudtrail_log_group.name

  pattern = "{ $.userIdentity.type = \"Root\" && $.userIdentity.invokedBy NOT EXISTS && $.eventType = \"AwsConsoleSignIn\" }" # 루트 계정 로그인 이벤트 감지

  metric_transformation {
    name      = "RootAccountUsage"
    namespace = "CloudTrailMetrics"
    value     = "1" # 루트 계정 사용 시 1로 설정
  }
}
...
resource "aws_cloudwatch_metric_alarm" "root_account_usage_alarm" {
  ...
  period              = 300
  statistic           = "Sum"
  threshold           = 1 # 임계값 1로 설정

  alarm_actions = [
    aws_sns_topic.cloudtrail_sns_topic.arn # SNS 알람과 연결
  ]
}
